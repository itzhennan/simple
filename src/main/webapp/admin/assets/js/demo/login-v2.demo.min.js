...
var currentIndex, lastIndex = 0, switchInt, handleLoginPageChangeBackground = function () {
    $(document).on("click", '[data-click="change-bg"]', function (b) {
        b.preventDefault();
        clickBGI($(this))
    })
}, clickBGI = function (a) {
    var b = "url(" + $(a).attr("data-img") + ")";
    $('[data-id="login-cover-image"]').css("background-image", b), $('[data-click="change-bg"]').closest("li").removeClass("active"), $(a).closest("li").addClass("active")
}, userLogin = function () {
        ...
        var a = Array.prototype.filter.call(b, function (c) {
            c.addEventListener("submit", function (e) {
                ...
                $IEC.ajaxCall({
                    ...
                    success: function (f) {
                        if (f.code == 0) {
                            $IEC.doHref(d + "/admin/html/calendar.html")
                        } else {
    ...
    return {
        init: function () {
            ...
            userLogin();
            currentIndex = 1;
            var a = $(".clearfix").children().length;
            var b = window.setInterval(function () {
                clickBGI($(".clearfix").children().eq(currentIndex).children().eq(0));
                currentIndex = currentIndex >= (a - 1) ? 0 : currentIndex + 1
            }, 5000)
        }
    }
}();