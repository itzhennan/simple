...
import cn.zznlin.simple.common.utils.bean.DBInfo;
...
public class DBInfoUtils {
    ...
    private final static String DATABASE_NAME = "simple";
    
    private static List<String> tableNameList = Lists.newArrayList();
    private static Map<String,List<DBInfo>> dbInfoMaps = Maps.newHashMap();
    ...
    private void getDatabaseMetaData() {
        try {
            if (dbMetaData == null) {
                ...
                String user = "zznlin";
        		String password = "ZZNLINmysql@959";
        		String url = "jdbc:mysql://45.76.214.145:3306/"+ DATABASE_NAME
...
//    }
     ...
     * 获得表或视图中的所有列信息
     */
    public void getTableColumns(String schemaName, List<String> tmpeTableNameList) {
 
        try {
        	
        	for (String tableName : tmpeTableNameList) {
	            ResultSet rs = dbMetaData.getColumns(null, schemaName, tableName, "%");
	            while (rs.next()) {
	...
	//                System.out.println("表名："+tableName_);
	                
	                DBInfo dbInfo = new DBInfo(tableName_, columnName, dataTypeName, columnSize, columnDef, remarks);
	                List<DBInfo> list = dbInfoMaps.get(tableName);
	                if(list == null){
	                	list = Lists.newArrayList();
	                }
	                list.add(dbInfo);
	                dbInfoMaps.put(tableName, list);
    ...
    public static void main(String[] args) {
    	DBInfoUtils metaData = new DBInfoUtils();
...
//        metaData.getAllTableList(null);
        ...
        metaData.getTableColumns("root", tableNameList);
        System.out.println(dbInfoMaps.toString());
